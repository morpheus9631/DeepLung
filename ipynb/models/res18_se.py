...
# forw 模組和 back 模組的迭代實作。path模組，其實就是反卷積模塊。
class Net(nn.Module):
    def __init__(self):
    ...
    def forward(self, x, coord):
        ...
        nodule_out = nodule_out.transpose(1,2).contiguous().view(
            nodule_size[0], nodule_size[2], nodule_size[3], 
            nodule_size[4], len(config['anchors']), 1
        )
        
        regress_out = regress_out.transpose(1,2).contiguous().view(
            regress_size[0], regress_size[2], regress_size[3],
